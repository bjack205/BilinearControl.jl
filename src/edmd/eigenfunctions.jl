function hermite(x::AbstractVector{T}; order::Vector{Int64} = [0, 0]) where T
...
function chebyshev(x::AbstractVector{T}; order::Vector{Int64} = [0, 0]) where T
...
function monomial(x::AbstractVector{T}; order::Vector{Int64} = [0, 0]) where T
...
sine(xk::AbstractVector; order=[1]) = sin.(order[1]*xk)
cosine(xk::AbstractVector; order=[1]) = cos.(order[1]*xk)
...
function build_eigenfunctions(X::VecOrMat{<:AbstractVector{<:AbstractFloat}}, 
                              ...
                              function_list::Vector{String}, order_list::Vector{Vector{Int64}})
