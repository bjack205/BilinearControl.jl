...
\begin{figure}[t]
  ...
  \caption{Effect of increased training data on controller performance. With just a few 
    ...
    \todo{combined this into a single plot? Add training time plot to the side?}}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.48\textwidth, height=5cm]{cartpole_mpc_train_time.tikz}
...
\begin{figure}
  \centering
  \includegraphics[width=1.0\textwidth, height=5cm]{cartpole_friction_mismatch.tikz}
  ...
  %   \begin{subfigure}[b]{0.40\textwidth}
    ...
    %   \begin{subfigure}[b]{0.59\textwidth}
      ...
      \begin{figure}
        ...
        % \begin{subfigure}[t]{0.45\textwidth}
        %   \includegraphics[width=\textwidth,height=5cm]{rex_planar_quadrotor_lqr_error_by_training_window.tikz}
        %   \caption{LQR stabilization error over varying range of distribution for sampling
        %   initial conditions. A training range fraction greater than $1$ indicates the
        %   distribution range is beyond that used to generate the training trajectories}
        %   \label{fig:rex_planar_quadrotor_lqr_error_by_training_window}
        % \end{subfigure}
        % \caption{ LQR controller robustness to varying initial conditions and equilibrium
        % offset. jDMD is much more robust overall with an ability to stabilize outside the
