...
const REX_PLANAR_QUADROTOR_DATA_FILE = joinpath(BilinearControl.DATADIR, "rex_planar_quadrotor_mpc_trajectory_data.jld2")
...
function generate_planar_quadrotor_data()
    ...
    jldsave(REX_PLANAR_QUADROTOR_DATA_FILE; 
...
function train_planar_quadrotor_models(num_lqr, num_mpc;  α=0.5, learnB=true, β=1.0, reg=1e-6)
    ...
    #############################################  
    mpc_lqr_traj = load(REX_PLANAR_QUADROTOR_DATA_FILE)
...
#############################################
## Tracking Success vs. Training Window
#############################################
...
jldsave(REX_PLANAR_QUADROTOR_RESULTS_FILE; percentages, res_training_range)
