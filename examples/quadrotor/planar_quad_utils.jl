...
function planar_quad_mpc_generalization(;save_to_file=true)
    ...
    errors = ThreadsX.map(percentages) do perc
        ...
        error_eDMD_loreg = mean(test_initial_conditions(
            model_real, model_eDMD_projected_1, x0_test, tf, tf, dt))
        error_jDMD_loreg = mean(test_initial_conditions(
            model_real, model_jDMD_projected_1, x0_test, tf, tf, dt))
        error_eDMD_hireg = mean(test_initial_conditions(
            model_real, model_eDMD_projected_2, x0_test, tf, tf, dt))
        error_jDMD_hireg = mean(test_initial_conditions(
    ...
    if save_to_file
        jldsave(joinpath(BilinearControl.DATADIR, 
            "rex_planar_quadrotor_mpc_training_range_results.jld2"); 
            percentages, 
            res_training_range
        )
    end
