...
function generate_zigzag_traj()
    ...
    solver = ALTROSolver(Problems.Quadrotor(:zigzag, MRP{Float64})..., projected_newton=false)
...
function train_quadrotor_models(num_lqr, num_mpc;  α=0.5, learnB=true, β=1.0, reg=1e-6)
...
function train_quadrotor_models_no_eDMD_reg(num_lqr, num_mpc;  α=0.5, learnB=true, β=1.0, reg=1e-6)
...
res = train_quadrotor_models(0, 30, α=0.5, β=1.0, learnB=true, reg=1e-6)
@show res.jDMD_err_avg
@show res.eDMD_err_avg

jldsave(REX_PLANAR_QUADROTOR_RESULTS_FILE; res, res_no_reg, )

#############################################
## Load results and models
#############################################

res = load(REX_PLANAR_QUADROTOR_RESULTS_FILE)["res"]
mpc_lqr_traj = load(joinpath(Problems.DATADIR, "rex_full_quadrotor_mpc_tracking_data.jld2"))
...
p_tracking = @pgf Axis(
    ...
    PlotInc({lineopts..., color="teal"}, Coordinates(x_ref, ry_ref)),
