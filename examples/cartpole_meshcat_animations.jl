...
include("cartpole/cartpole_utils.jl")

const CARTPOLE_RESULTS_FILE = joinpath(BilinearControl.DATADIR, "cartpole_meshcat_results.jld2")
...
function generate_trajectories(;
    ...
    model_projected = BilinearControl.ProjectedEDMDModel(model_edmd)
    ...
    model_projected = BilinearControl.ProjectedEDMDModel(model_jdmd)
    mpc = generate_stabilizing_mpc_controller(model_projected, t_sim, dt; Nt, ρ)
...
set_cartpole!(vis["nominal_cart"]; color=nominal_color, color2=nominal_color)
visualize!(vis["nominal_cart"], model, X_nom_01[1])
...
##
visualize_multiple(vis["edmd_cart"], vis["jdmd_cart"], vis["nominal_cart"],
            model, 7.0, X_edmd_01, X_jdmd_01, X_nom_01)
...
set_cartpole!(vis["nominal_cart"]; color=nominal_color, color2=nominal_color)
visualize!(vis["nominal_cart"], model, X_nom_05[1])
...
##
visualize_multiple(vis["edmd_cart"], vis["jdmd_cart"], vis["nominal_cart"],
            model, 7.0, X_edmd_05, X_jdmd_05, X_nom_05)
