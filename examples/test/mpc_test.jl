...
Rmpc = Diagonal(fill(1e-2,1))
Qfmpc = Diagonal([1e1,1e1,1e0,1e0])


mpc = EDMD.TrackingMPC(dmodel, X_ref, U_ref, T_ref, Qmpc, Rmpc, Qfmpc, Nt=41)
mpc2 = EDMD.TrajectoryTracker(dmodel, X_ref, U_ref, T_ref, Qmpc, Rmpc, Qfmpc, Nt=21)
# mpc2 = EDMD.TrajectoryTracker(dmodel, X_ref, U_ref, T_ref, Qmpc, Rmpc, Qfmpc, Nt=101)
tvlqr = EDMD.TVLQRController(dmodel, Qmpc, Rmpc,Qfmpc, X_ref, U_ref, T_ref)

##
Qmpc = Diagonal([0.5, 10.0, 0.1, ])
Rmpc = Diagonal(fill(1e-3,1))
Qfmpc = 10 * Qmpc
mpc2 = EDMD.TrajectoryTracker(dmodel, X_ref, U_ref, T_ref, Qmpc, Rmpc, Qfmpc, Nt=31)

t_sim = 6.0 
dx = [0.0,0,0,0.00]
X_sim,_,T_sim = simulatewithcontroller(dmodel_true, mpc2, X_ref[1] + dx, t_sim, dt)
# X_sim,_,T_sim = simulatewithcontroller(dmodel, tvlqr, X_ref[1]+dx, t_sim, dt)
# X_sim,_,T_sim = simulatewithcontroller(dmodel, mpc, X_ref[1]+dx, t_sim, dt)
...
f = [zeros(n)*0 for k=1:N-1]
Q = [Diagonal(rand(n)) for k = 1:N]
R = [Diagonal(rand(m)) for k = 1:N-1]
q = [randn(n)*0 for k = 1:N]
r = [randn(m)*0 for k = 1:N-1]
x0 = randn(n)
...
c = Aqp*res.x - b 
c[1:n] ≈  x0 - X2[1] 
c[11:20] ≈ A[1]*X2[1] + B[1] * U2[1] + f[1] - X2[2]
c[1n+1:2n] ≈ A[1] * X2[1] + B[1] * U2[1] + f[1] - X2[2]
c[2n+1:3n] ≈ A[2] * X2[2] + B[2] * U2[2] + f[2] - X2[3]
