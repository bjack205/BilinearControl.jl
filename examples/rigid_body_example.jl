...
include("rigid_body_model.jl")
include(joinpath(@__DIR__, "../test/models/rotation_utils.jl"))

##
mass = 2.0
J = I(3)
model = RigidBodyDynamics(mass, diag(J)...)
n,m = RD.dims(model)
@test n == 87
@test m == 6
@test base_state_dim(model) == 18
...
function buildrigidbodyproblem()
    # Model
    mass = 2.9
    J = I(3)
    model = RigidBodyDynamics(mass, diag(J)...)
