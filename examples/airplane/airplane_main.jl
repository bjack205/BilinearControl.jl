...
results = map(alpha) do N
    @show N
    closed_loop_res = test_airplane(train_airplane(10; α=N)...)
    open_loop_res = test_airplane_open_loop(train_airplane(10; α=N)...)
    next!(prog)
...
jldsave(joinpath(BilinearControl.DATADIR, "airplane_model_error_results.jld2"); results_model_pred)
...
num_train = load(AIRPLANE_RESULTS)["num_train"] 
alpha = 0.0:0.05:0.9
results_model_pred = load(joinpath(BilinearControl.DATADIR, "airplane_model_error_results.jld2"))["results_model_pred"]
